        ...
        void UpdateDelayExposure()
        {
            ...
            else if (this._manualModeRb.Checked)
            {
                ...
                try
                {
                    ...
                    episcan.Sensor.Exposure = exposure;
                    //var capture = episcan.CaptureAverage(4);
                    
                    //Cv2.ImWrite($"C:\\episcandemo-win-taka20190529-001\\episcandemo-win-taka\\datas\\" + episcan.Sensor.Delay.ToString() + episcan.Sensor.Exposure.ToString() + ".png", capture);
                }
                catch (Exception)
                {
            ...
            else if (this._delayModeRb.Checked)
            {
                ...
                for (float delayvalue = 1000f; delayvalue <= 20000f; delayvalue += 10f)
                {
                    episcan.Sensor.Exposure = exposure;
                    ...
                    Console.WriteLine("{0},{1}", "Synchronisation Delay", episcan.Sensor.Delay.ToString());
                    //var img = await episcan.CaptureAverage();
                    //Cv2.ImWrite('td{0}_te_{1}_Pc_{2}.png',Capture)
